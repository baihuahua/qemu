...
int tcmu_get_cfgfs_int(const char *path)
{
	...
	if (fd == -1) {
		printf("Could not open configfs to read attribute %s: %s\n",
			 path, strerror(errno));
		return -errno;
	}
	...
	close(fd);
	if (ret == -1) {
		printf("Could not read configfs to read attribute %s: %s\n",
			 path, strerror(errno));
		return -errno;
	}
...
char *tcmu_get_cfgfs_str(const char *path)
{
	...
	val = strdup(buf);
	if (!val) {
		printf("could not copy buffer %s\n", buf);
		return NULL;
	}
...
int tcmu_set_cfgfs_str(const char *path, const char *val, int val_len)
{
	...
	if (fd == -1) {
		printf("Could not open configfs to write attribute %s: %s\n",
			 path, strerror(errno));
		return -errno;
	}
	...
	close(fd);
	if (ret == -1) {
		printf("Could not write configfs to write attribute %s: %s\n",
			 path, strerror(errno));
		return -errno;
	}
