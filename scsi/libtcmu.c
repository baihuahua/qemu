...
static void remove_device(struct tcmulib_context *ctx, char *dev_name, const char *cfgstring);
static int handle_netlink(struct nl_cache_ops *unused, struct genl_cmd *cmd,
			  struct genl_info *info, void *arg);

static struct genl_cmd const tcmu_cmds[] = {
	{
...
static struct genl_ops tcmu_ops = {
	.o_name		= "TCM-USER",
	.o_cmds		= tcmu_cmds,
	.o_ncmds	= ARRAY_SIZE_TCMU(tcmu_cmds),
};
...
free_msg:
	...
	if (ret < 0)
		printf("Could not send netlink cmd %d\n", reply_cmd);
	return ret;
}
			   ...
			   struct genl_info *info)
{
	...
	if (!dev) {
		printf("Could not reconfigure device %s: not found.\n",
			 dev_name);
		return -ENODEV;
	}

	if (!dev->handler->reconfig) {
		printf("Reconfiguration is not supported with this device.\n");
		return -EOPNOTSUPP;
	}
	...
	if (info->attrs[TCMU_ATTR_DEV_CFG]) {
		...
		printf( "Unknown reconfig attr. Try updating libtcmu.\n");
		return -EOPNOTSUPP;
	}

	ret = dev->handler->reconfig(dev, &cfg);
	if (ret < 0) {
		printf("Handler reconfig failed with error %d.\n",
			     ret);
		return ret;
	}
			  ...
			  struct genl_info *info, void *arg)
{
	...
	if (!info->attrs[TCMU_ATTR_MINOR] || !info->attrs[TCMU_ATTR_DEVICE]) {
		printf("TCMU_ATTR_MINOR or TCMU_ATTR_DEVICE not set, dropping netlink command.\n");
		return 0;
	}

	if (version > 1 && !info->attrs[TCMU_ATTR_DEVICE_ID]) {
		printf("TCMU_ATTR_DEVICE_ID not set in v%d cmd %d, dropping netink command.\n", version, cmd->c_id);
		return 0;
	}
	...
	case TCMU_CMD_RECONFIG_DEVICE:
		...
		break;
	default:
		printf("Unknown netlink command %d. Netlink header received version %d. libtcmu supports %d\n",
...
free_msg:
	...
	if (ret < 0)
		printf("Could not set features. Error %d\n", ret);
		      ...
		      char *dev_name, char *cfgstring)
{
	...
	if (dev->handler->check_config &&
		...
		printf("check_config failed for %s because of %s\n", dev->dev_name, reason);
		free(reason);
		goto err_free;
	}
...
static void remove_device(struct tcmulib_context *ctx,
			  char *dev_name, const char *cfgstring)
{
	struct tcmu_device *dev;
	int i, ret;
	...
	ret = close(dev->fd);
	if (ret != 0) {
		printf("could not close device fd %s: %d\n", dev_name, errno);
	}
	ret = munmap(dev->map, dev->map_len);
	if (ret != 0) {
		printf("could not unmap device %s: %d\n", dev_name, errno);
	}
...
void tcmulib_close(struct tcmulib_context *ctx)
{
	...
	if (ret != 0) {
		printf("genl_unregister_family failed, %d\n", ret);
	}
	free(ctx);
}
...
void tcmulib_processing_start(struct tcmu_device *dev)
{
	...
	if (r == -1 && errno != EAGAIN)
		printf("failed to read device /dev/%s, %d\n",
			 dev->dev_name, errno);
}
...
void tcmulib_processing_complete(struct tcmu_device *dev)
{
	...
	if (r == -1 && errno != EAGAIN)
		printf("failed to write device /dev/%s, %d\n",
			 dev->dev_name, errno);
}
...
void tcmulib_cleanup_cmdproc_thread(struct tcmu_device *dev)
{
	...
	if (!found) {
		printf("could not remove a device: not found\n");
		return;
	}
...
void tcmulib_cleanup_all_cmdproc_threads(void)
{
	struct tcmu_thread *thread;
	darray_foreach(thread, g_threads) {
		cancel_thread(thread->thread_id);
